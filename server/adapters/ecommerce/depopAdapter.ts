/**
 * Depop Adapter
 * API Base: https://api.depop.com/v2
...
const API_BASE = "https://api.depop.com/v2";
...
export class DepopAdapter implements EcommercePlatformAdapter {
  ...
  async verifyConnection(credentials: AdapterCredentials): Promise<StoreInfo> {
    return {
      platformId: credentials.platformAccountId || "depop",
      name: "Depop Store",
      domain: "depop.com",
      currency: "USD",
      status: "active",
  ...
  async healthCheck(credentials: AdapterCredentials): Promise<{ healthy: boolean; message: string; latencyMs: number }> {
    ...
    try {
      ...
      return {
        healthy: true,
        message: "Depop connection verified",
        latencyMs: Date.now() - start,
      };
    } catch (error) {
      return {
        healthy: false,
        message: `Depop connection failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        latencyMs: Date.now() - start,
      };
    }
  }
