/**
 * Bonanza Adapter
 * API Base: https://api.bonanza.com
 * Auth: Token-based
 */
...
const API_BASE = "https://api.bonanza.com";
...
export class BonanzaAdapter implements EcommercePlatformAdapter {
  ...
  async verifyConnection(credentials: AdapterCredentials): Promise<StoreInfo> {
    return {
      platformId: credentials.platformAccountId || "bonanza",
      name: "Bonanza Store",
      domain: "bonanza.com",
      currency: "USD",
      status: "active",
  ...
  async healthCheck(credentials: AdapterCredentials): Promise<{ healthy: boolean; message: string; latencyMs: number }> {
    ...
    try {
      ...
      return {
        healthy: true,
        message: "Bonanza connection verified",
        latencyMs: Date.now() - start,
      };
    } catch (error) {
      return {
        healthy: false,
        message: `Bonanza connection failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        latencyMs: Date.now() - start,
      };
    }
  }
