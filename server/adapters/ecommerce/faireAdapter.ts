/**
 * Faire Adapter
 * API Base: https://api.faire.com/v1
 * Auth: API Token
 */
...
const API_BASE = "https://api.faire.com/v1";
...
export class FaireAdapter implements EcommercePlatformAdapter {
  ...
  async verifyConnection(credentials: AdapterCredentials): Promise<StoreInfo> {
    return {
      platformId: credentials.platformAccountId || "faire",
      name: "Faire Store",
      domain: "faire.com",
      currency: "USD",
      status: "active",
  ...
  async healthCheck(credentials: AdapterCredentials): Promise<{ healthy: boolean; message: string; latencyMs: number }> {
    ...
    try {
      ...
      return {
        healthy: true,
        message: "Faire connection verified",
        latencyMs: Date.now() - start,
      };
    } catch (error) {
      return {
        healthy: false,
        message: `Faire connection failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        latencyMs: Date.now() - start,
      };
    }
  }
