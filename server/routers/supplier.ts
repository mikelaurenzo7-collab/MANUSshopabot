    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (const [supplierId, items] of Object.entries(bySupplier)) {
        ...
        const poId = await db.createPurchaseOrder({
          ...
          totalCents,
          status: "draft",
          notes: "Auto-generated by Store Bot low-stock detection",
        });
        ...
        await db.createNotification({
          ...
          message: `Store Bot drafted a $${(totalCents / 100).toFixed(2)} PO for ${items.length} product(s) running low. Review and approve.`,
          actionUrl: "/supplier",
