/**
 * InfraTopology.tsx — Hard-coded infrastructure topology view.
 *
 * Renders the TiDB / BullMQ / Sharp / tRPC / bots topology that used to
 * be the homepage Command Center. After the Command Center reimagining
 * this view became operator-only and is hosted inside Platform Health.
 *
 * The component is self-contained: it owns its data fetches, error/loading
 * states, ReactFlow nodes/edges, and the inspector panel. Drop into any
...
import {
  Loader2, Database, Zap, Bot, Box,
  Globe, Workflow, Package, Megaphone,
...
export default function InfraTopology() {
  ...
  const {
    data: metrics, isLoading: metricsLoading, error: metricsError, refetch: refetchMetrics,
  } = trpc.dashboard.metrics.useQuery({}, { refetchInterval: 30000 });
  const {
    data: agentStatus, error: agentError, refetch: refetchAgentStatus,
  ...
  const buildNodes = useCallback((): Node[] => [
    { id: "db", type: "custom", position: { x: 40, y: 320 },
      data: { label: "TiDB / Drizzle", value: "ONLINE",
        icon: <Database className="w-4 h-4 text-emerald-400" />, status: "ok",
        details: { Region: "AWS us-east-1", Pool: "Active", "Schema Version": "v14" } } },
    { id: "workflows", type: "custom", position: { x: 360, y: 120 },
      data: { label: "Workflow Engine", value: totalRunning > 0 ? `${totalRunning} RUNNING` : "IDLE",
        icon: <Workflow className="w-4 h-4 text-amber-400" />, status: totalRunning > 0 ? "active" : "ok",
        details: { "Active Tasks": totalRunning, "Completed": totalCompleted, "Engine": "State Machine v2" } } },
    { id: "queue", type: "custom", position: { x: 720, y: 120 },
      data: { label: "BullMQ Queue", value: "PROCESSING",
        icon: <Layers className="w-4 h-4 text-violet-400" />, status: "active",
        details: { "Retry Logic": "Exponential Backoff", "Max Attempts": 5, "Dead Letter": "Enabled" } } },
    { id: "connectors", type: "custom", position: { x: 360, y: 320 },
      data: { label: "Integrations", value: `${totalConnected} LINKED`,
        icon: <Globe className="w-4 h-4 text-sky-400" />, status: totalConnected > 0 ? "ok" : "idle",
        details: { "Stores": connSummary?.stores || 0, "Social Accounts": connSummary?.socialAccounts || 0, "Adapters": 13 } } },
    { id: "revenue", type: "custom", position: { x: 360, y: 520 },
      data: { label: "Revenue Stream", value: `$${revenue}`,
        icon: <TrendingUp className="w-4 h-4 text-emerald-400" />, status: "ok",
        details: { "Total Orders": metrics?.totalOrders || 0, "Active Products": metrics?.activeProducts || 0, "Stores": stores?.length || 0 } } },
    { id: "builder", type: "custom", position: { x: 720, y: 320 },
      data: { label: "Launch mode", value: "AUTONOMOUS",
        icon: <Bot className="w-4 h-4 text-cyan-400" />, status: "active",
        details: { Role: "Architect", Workflows: "Niche Research, Product Sourcing, Store Setup", Status: "Listening" } } },
    { id: "merchant", type: "custom", position: { x: 720, y: 440 },
      data: { label: "Operator mode", value: "AUTONOMOUS",
        icon: <Package className="w-4 h-4 text-violet-400" />, status: "active",
        details: { Role: "Merchant", Workflows: "Fulfillment, Inventory Sync, Pricing", Status: "Listening" } } },
    { id: "social", type: "custom", position: { x: 720, y: 560 },
      data: { label: "Growth mode", value: "AUTONOMOUS",
        icon: <Megaphone className="w-4 h-4 text-pink-400" />, status: "active",
        details: { Role: "Social", Workflows: "Ad Campaigns, Social Posts, Email Recovery", Status: "Listening" } } },
    { id: "sharp", type: "custom", position: { x: 1060, y: 120 },
      data: { label: "Image Pipeline", value: "SHARP ACTIVE",
        icon: <Zap className="w-4 h-4 text-yellow-400" />, status: "ok",
        details: { Format: "WebP / AVIF", Quality: 80, Transforms: "Resize, Crop, Optimize" } } },
    { id: "server", type: "custom", position: { x: 1060, y: 320 },
      data: { label: "tRPC Server", value: "SERVING",
        icon: <Server className="w-4 h-4 text-white/50" />, status: "ok",
    ...
    return (
      <div className="flex items-center justify-center bg-terminal-bg py-16 rounded-2xl">
        <div className="text-center rounded-2xl border border-red-500/20 bg-red-500/[0.03] px-8 py-7 shadow-[0_0_40px_rgba(239,68,68,0.08)]">
          ...
          <p className="text-xs font-bold uppercase tracking-widest text-red-400 mb-2">Topology Error</p>
          <p className="mx-auto max-w-sm text-[11px] leading-relaxed text-white/35">
            {(metricsError || agentError)?.message || "Failed to load topology data"}
          </p>
          <button
            type="button"
      ...
      <div className="flex flex-1 min-h-0" style={{ minHeight: 0 }}>
        ...
        <aside className="w-[300px] h-full shrink-0 border-l border-white/[0.06] bg-surface-deep/82 backdrop-blur-2xl flex flex-col z-20">
            ...
            {selectedNode ? (
              <>
                ...
                {selectedNode.data.details && (
                  <div>
                    <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-white/20 mb-2 pb-1.5 border-b border-white/[0.05]">Live Telemetry</p>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center px-4 py-12">
                ...
                <p className="text-[9px] text-white/15 mt-2 leading-relaxed max-w-[180px]">Click any canvas entity to inspect its telemetry and live data</p>
              </div>
            )}
          </div>
          ...
          <div className="border-t border-white/[0.05] px-4 py-2.5 flex items-center justify-between">
            <span className="text-[9px] font-mono text-white/20">{nodes.length} nodes · {edges.length} edges</span>
            <span className="text-[9px] font-mono text-white/20">{stores?.length || 0} stores</span>
          </div>
        </aside>
      </div>
    </div>
